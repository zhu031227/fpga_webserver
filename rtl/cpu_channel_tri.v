...
module cpu_channel_tri #(
  ...
  // ============================================================
  // eth1 RX → eth2 TX bridge forwarding path
  // ============================================================
  ...
  wire                          eth1_fwd_wpkt_push_raw;  // before whitelist gate
  wire                          eth1_fwd_wpkt_push;  // after whitelist gate
  ...
  // ============================================================
  // eth2 RX → eth1 TX bridge forwarding path (unconditional)
  // ============================================================
  ...
  // ============================================================
  // Whitelist: SrcMAC extraction from eth1 header (first 14 bytes)
  // ============================================================
  ...
  // ============================================================
  // eth1 MAC header extraction (SrcMAC for whitelist)
  // ============================================================
  always @(negedge reset_l or posedge clk) begin
    if (reset_l == 1'b0) begin
      mac1_src_mac    <= 48'b0;
  ...
  // ============================================================
  // Whitelist lookup trigger (eth1 on header completion)
  // ============================================================
  always @(negedge reset_l or posedge clk) begin
    if (reset_l == 1'b0) begin
      wl_lookup_req    <= 1'b0;
  ...
  // ============================================================
  // eth0 RX → ram2pktfifo_int → CPU (all packets)
  // ============================================================
  ram2pktfifo_int #(
  ...
  // Whitelist gate: forward if match OR whitelist disabled with default_pass
  assign eth1_fwd_wpkt_push = eth1_fwd_wpkt_push_raw &&
      (wl_result_match || (!whitelist_en && default_pass));

  // eth1→eth2 forwarding FIFO (single clock 125MHz)
  package_fifo_v2 #(
      .dual_clock(0),
      .addr_width(cpu_buf_addr_width),
