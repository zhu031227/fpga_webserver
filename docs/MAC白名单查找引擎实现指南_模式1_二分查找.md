...
> **定位说明（2026-08-29）**：项目实施主线定为 **模式 0 → 模式 2（布谷鸟哈希）**，见《MAC白名单查找引擎实现指南_模式2_布谷鸟哈希.md》；本篇为**备选路线**——内容不变、仍自包含可独立实施，前置条件仍为模式 0 收官，与模式 2 无先后依赖。
...
| 2026-08-29 | R02a（ED003R02-B） | 主线调整：项目主线定为 模式 0 → 模式 2，本篇定位改为备选路线（头部加定位说明），内容不变 | Claude |
...
C 固件经 LCPU 总线 → `reg_webserver`（SubBus 0x5000~0x5FFF 段译码）→ `ramintf` → 白名单 cfg 口（50MHz 域）。上游 LCPU 写/读时序与白名单 cfg 口电平采样波形见模式 0 文档图 1/图 2/图 3；cfg 口上看到的核心事实（图 2）：
