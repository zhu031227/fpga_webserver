...
module tb_webserver;
  ...
  task gmii_send_idle;
    ...
    integer i;
    begin
      for (i = 0; i < cycles; i = i + 1) gmii_send_byte(8'h00, 1'b0);
    end
  endtask
  ...
  task gmii_send_frame;
    ...
    begin
      ...
      // Frame data bytes
      for (i = 0; i < n_bytes; i = i + 1) gmii_send_byte(pkt[i], 1'b1);
  ...
  //-------------------------------------------------------------------
  always @(posedge clk_50m_in) begin
    if (u_dut.u_webserver.eth0_mac_tx_sop) $display("[%0t ns] MAC TX SOP", $time);
    if (u_dut.u_webserver.eth0_mac_tx_eop) $display("[%0t ns] MAC TX EOP", $time);
  end
  ...
  //-------------------------------------------------------------------
  always @(posedge clk_50m_in) begin
    if (cpu_req) $display("[%0t ns] CPU REQ addr=0x%08h", $time, cpu_addr);
    if (cpu_ack) $display("[%0t ns] CPU ACK", $time);
  end
endmodule
