module lcpu_riscv_wrapper #(
  ...
  // LCPU / JTAG master: simulation BFM only (sim_mod is always 1)
  // The real-hardware lcpu_top path is stripped to avoid pulling in
  // all ip_lcpu dependencies (jtag_cpu_xilinx, uart_lcpu_top, etc.).
  generate
    if (sim_mod == 0) begin : lcpu_real
      // Not used in simulation — stub out the outputs
      assign jtag_req     = 1'b0;
      assign jtag_rhwl    = 1'b0;
      assign jtag_wdata   = 32'b0;
      assign jtag_address = 32'b0;
      assign uart_tx      = 1'b1;
